    ...
    node(buildLabel) {
        ...
        container(name: 'jdk11', shell: '/bin/bash') {
            ...
            stage('Sonar scan') {
                ...
                ./gradlew -Dsonar.login=${SONARQUBE_USER} -Dsonar.password=${SONARQUBE_PASSWORD} -Dsonar.host.url=http://sonarqube.mooc-one-iks-cluster.us-east.containers.appdomain.cloud/ -Dsonar.jacoco.reportPath=./build/jacoco/test.exec -Dsonar.projectName=lsteck.mooc-java-spring-tese sonarqube
                '''
            }
        }
