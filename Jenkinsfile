    ...
    node(buildLabel) {
        ...
        container(name: 'jdk11', shell: '/bin/bash') {
            ...
            stage('Sonar scan') {
                ...
                ./gradlew -Dsonar.login=${SONARQUBE_USER} -Dsonar.password=${SONARQUBE_PASSWORD} -Dsonar.host.url=http://sonarqube.mooc-one-iks-cluster.us-east.containers.appdomain.cloud/ -Dsonar.junit.reportPaths=./build/reports/jacoco/test/jacocoTestReport.xml sonarqube
                '''
            }
        }
